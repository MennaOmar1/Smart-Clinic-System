...
class ElevenLabsSchedulingServiceTests(unittest.TestCase):
    def setUp(self):
        ...
        user = User(id=1, name="Dr. Ahmed Hassan", email="ahmed@example.com", role="doctor")
        doctor = Doctor(id=1, user_id=1, specialization="Cardiology")
    ...
    def test_reports_available_for_valid_future_cairo_slot(self):
        result = ElevenLabsSchedulingService.check_availability(
            self.db,
            doctor_id=1,
            requested_time=datetime(2026, 5, 12, 10, 0, tzinfo=CAIRO),
            now=datetime(2026, 5, 11, 12, 0, tzinfo=CAIRO),
        )

        self.assertTrue(result["is_available"])
        self.assertEqual(result["error_code"], None)
        self.assertEqual(result["normalized_time"], "2026-05-12T10:00:00+03:00")

    def test_rejects_non_30_minute_slot_alignment(self):
        result = ElevenLabsSchedulingService.check_availability(
            self.db,
            doctor_id=1,
            requested_time=datetime(2026, 5, 12, 10, 15, tzinfo=CAIRO),
            now=datetime(2026, 5, 11, 12, 0, tzinfo=CAIRO),
        )

        self.assertFalse(result["is_available"])
        self.assertEqual(result["error_code"], "INVALID_SLOT_ALIGNMENT")

    def test_booking_creates_conflict_free_appointment_and_blocks_duplicate(self):
        start = datetime(2026, 5, 12, 10, 0, tzinfo=CAIRO)
        ...
        self.assertIsInstance(created["appointment_id"], int)
        stored = self.db.query(Appointment).filter(Appointment.id == created["appointment_id"]).one()
        self.assertEqual(stored.start_time, start.replace(tzinfo=None))
        self.assertEqual(stored.end_time, start.replace(tzinfo=None) + timedelta(minutes=30))

        with self.assertRaises(ElevenLabsSchedulingError) as raised:
        ...
        self.assertEqual(raised.exception.payload["error_code"], "SLOT_CONFLICT")
        self.assertFalse(raised.exception.payload["booking_success"])
...
class ElevenLabsRouteValidationTests(unittest.TestCase):
    ...
    def test_booking_validation_error_keeps_elevenlabs_mapping_paths(self):
        ...
        self.assertEqual(body["booking_confirmation"], body["message"])
        self.assertEqual(body["appointment_id"], None)
    ...
    def test_availability_validation_error_keeps_elevenlabs_mapping_paths(self):
        ...
        self.assertEqual(body["availability_message"], body["message"])
        self.assertEqual(body["error_code"], "VALIDATION_ERROR")
