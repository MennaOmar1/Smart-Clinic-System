...
# ① GEMINI CONFIG
# ═══════════════════════════════════════════════════════════
GEMINI_API_KEY = "AIzaSyDw-Aly9bbQXFdgDxby4IjPeIU84cAgBHs"  # ضع مفتاحك هنا
...
# ═══════════════════════════════════════════════════════════
# ② SYSTEM PROMPT (محسّن مع أمثلة شاملة)
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
# ④ TEXT FUNCTIONS (normalization, extraction, etc.)
# ═══════════════════════════════════════════════════════════
...
def parse_clinical_plan(raw_text: str) -> ClinicalPlan:
    ...
    # لا نفرغ ingredients هنا حتى نتمكن من عرضها للحالات الطارئة عند الطلب المؤقت
    return plan
...
# ═══════════════════════════════════════════════════════════
# ⑪ MAIN RAG FUNCTION (مع خاصية العلاج المؤقت في الطوارئ)
# ═══════════════════════════════════════════════════════════
def rag(query: str, history: list) -> str:
    # التحقق من تكرار الطوارئ ومنع الحلقات
    last_bot_message = ""
    if history and len(history) > 0:
        for msg in reversed(history):
            if msg.get("role") == "assistant":
                ...
                break
    # الكلمات التي تدل على طلب علاج مؤقت في الطوارئ
    ...
    # إذا كان آخر رد فيه طوارئ والمستخدم يطلب علاج مؤقت، نتعامل كـ "caution" مؤقتاً
    if "روح الطوارئ فوراً" in last_bot_message and asks_for_temporary:
        # نعيد بناء السياق مع flag مؤقت
        ctx = extract_context(query, history)
        # نمرر الطلب كأنه غير طارئ (لكننا سنضيف تحذيراً)
        # نأخذ الخطة الأخيرة من الذاكرة؟ لكن لا توجد خطة مخزنة. الحل:
        # نطلب Gemini مرة أخرى (مع تعديل بسيط في البرومبت) أو نستخدم آخر خطة تم إنتاجها؟
        # الأسهل: نعيد استدعاء Gemini لكن مع إضافة تعليق في augmented بأن المريض يريد علاجاً مؤقتاً رغم الطوارئ.
        gemini_messages = []
        for msg in history:
    ...
    # السلوك العادي (غير طلب علاج مؤقت)
    ctx = extract_context(query, history)
    ...
    if plan.escalation_level == "urgent":
        # إذا كانت الحالة طارئة ولم يطلب المريض علاجاً مؤقتاً (تم فحصه أعلاه)، نرسل تحذير الطوارئ فقط
